...
const updateWordCloudByElement = (element) => {
  ...
  console.log(`按${element}属性筛选出${elementHerbs.length}种药材`)
  ...
  const newOption = {
    title: {
      text: `${element}行药材功效词云`,
      subtext: `统计自${elementHerbs.length}种药材的功效`,
      left: 'center',
      textStyle: {
        color: '#3A2E39',
        fontFamily: '楷体',
...
const resetWordCloud = () => {
  ...
  const option = {
    title: {
      text: '药材功效总体词云',
      subtext: `统计自${herbalStore.herbs.length}种药材的功效`,
      left: 'center',
      textStyle: {
        color: '#3A2E39',
        fontFamily: '楷体',
...
function updateTimelineByElement(element) {
  ...
  const option = {
    title: {
      text: `${element}行药材历史文献记载统计`,
      subtext: `按朝代统计${element}行属性${elementHerbs.length}种药材的文献记载数量`,
      left: 'center',
      top: 10,
      textStyle: {
        color: '#3A2E39',
...
const updateMapByElement = (element) => {
  ...
  const newOption = {
    title: {
      text: `${element}行药材产地分布`,
      subtext: `共${elementHerbs.length}种药材，分布在${Object.keys(provinceData).length}个省份`
    },
    visualMap: {
      inRange: {
        color: [
