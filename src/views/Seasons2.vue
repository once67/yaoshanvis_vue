...
const selectedSeason = ref(monthsData[new Date().getMonth()].阴历月份); // 默认节气
...
onMounted(() => {
  ...
  selectedSeason.value = monthsData[currentMonth].阴历月份;
});
</script>
...
.seasons-container {
  ...
  color: #333;
  overflow: hidden;
  background-image: url('/img/background/ancient-herbs.jpg');
...
.ancient-paper-bg {
  ...
  width: 100%;
  height: 100%;
  background: url('/img/textures/paper-texture.png');
  opacity: 0.08;
  z-index: 0;
  pointer-events: none;
}
...
.tech-lines {
  ...
  width: 100%;
  height: 100%;
  background: url('/img/textures/grid-lines.png');
  opacity: 0.05;
  z-index: 0;
  pointer-events: none;
}
